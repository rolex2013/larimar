...
def companies(request, pk=0, razdel='projects'):
    ...
    return render(
        ...
        {
            'nodes':
            nodes.select_related("author", "currency", "structure_type",
                                 "type"),
            'len_list':
            len(nodes),
            'current_company':
...
def staffs(request, stafflistid=0, pk=0):
    ...
    comps = request.session['_auth_user_companies_id']
    current_stafflist = StaffList.objects.filter(
        id=stafflistid).select_related("author", "company", "currency",
                                       "type").first()
    current_company = Company.objects.filter(
        id=current_stafflist.company_id).select_related(
            "author", "currency", "structure_type", "type").first()
    ...
    nodes = Staff.objects.filter(
        Q(dateend__gte=datetime.now()) | Q(dateend=None),
        datebegin__lte=datetime.now(),
        is_active=True,
        stafflist_id=stafflistid).select_related("author", "stafflist",
                                                 "user").order_by()
...
def summaries(request, pk=0):
    template_name = 'stafflist_detail.html'
    if pk == 0:
        summaries_list = Summary.objects.filter(is_active=True,
                                                stafflist__is_active=True,
                                                stafflist__is_vacancy=True)
    else:
        summaries_list = Summary.objects.filter(stafflist_id=pk,
                                                is_active=True,
                                                stafflist__is_active=True,
                                                stafflist__is_vacancy=True)
    summaries_list = summaries_list.select_related("stafflist").order_by(
        '-datecreate')
    param = 'summaries'
    comps = request.session['_auth_user_companies_id']
    current_stafflist = StaffList.objects.filter(id=pk).select_related(
...
def summary_detail(request, pk):
    ...
    current_summary = Summary.objects.filter(
        id=pk).select_related("stafflist").first()
    ...
    if request.user.is_authenticated:
        ...
        if place == 0:
            content_list = Content.objects.filter(
                ...
                place_id=2).annotate(cnt=Count('id'))
        else:
            content_list = Content.objects.filter(
                is_active=True,
                datebegin__lte=datetime.now(),
                ...
                company__is_active=True,
                company__in=companies_id,
                place_id=3).annotate(cnt=Count('id'))
...
def publiccontents(request):
    ...
    content_list = Content.objects.filter(
        ...
        place_id=1).select_related("author", "type",
                                   "place").annotate(cnt=Count('id'))
    param = 'publiccontent'
    return render(request, template_name, {
        'content_list': content_list,
        'param': param,
...
class ContentList(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        if self.request.user.is_authenticated:
            ...
            # добавляем к контексту сессионный массив с id компаний, доступными этому авторизованному юзеру
            # button_company_select = 'Сменить организацию'
            context['user_companies'] = self.request.session[
                '_auth_user_companies_id']
            # context['button_company_select'] = button_company_select
            return context
...
class ContentDetail(DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if self.request.user.is_authenticated:
            context['button_content_create'] = _(
                'Добавить')  #button_company_create
...
def userroles(request, companyid=1, pk=1):
    ...
    roles = UserCompanyComponentGroup.objects.filter(
        user_id=pk, company_id=companyid, is_active=True).select_related(
            "company", "component", "group",
            "user").order_by('component_id', 'group_id')
    print(pk, roles)
    ...
    return render(
        request, 'company_user_detail.html', {
            'nodes': roles,
...
def userrole_delete(request):
    ...
    roles = UserCompanyComponentGroup.objects.filter(
        user_id=role.user_id, company_id=role.company_id,
        is_active=True).select_related("company", "component", "group",
                                       "user").order_by(
    ...
    button_companyuserrole_create = _('Добавить')
    return render(
        request, 'company_user_roles_list.html', {
            'nodes': roles,
            'componentlist': componentlist,
            'grouplist': grouplist,
            'button_companyuserrole_create': button_companyuserrole_create,
