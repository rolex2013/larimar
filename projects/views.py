...
from projects.models import (
    ...
    TaskComment,
    ProjectFile,
)  # , ProjectStatusLog, TaskStatusLog
...
def projects(request, companyid=0, pk=0):
    ...
    if pk == 0:
        ...
        # tree_project_id = current_project.tree_id
        if (
            currentuser == current_project.author_id
            or currentuser == current_project.assigner_id
        ):
            obj_files_rights = 1
    ...
    if currentuser == current_company.author_id:
        ...
        button_project_create = _("Добавить")
    if current_company.id in comps:
        button_project_create = _("Добавить")

    project_list = project_list.select_related(
        "company", "author", "assigner", "status", "type", "structure_type", "currency"
    )  # .prefetch_related(
    # "members")
...
class ProjectCreate(AddFilesMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        historyjson = {
            str(_("Проект")): self.object.name,
            str(_("Статус")): self.object.status.name,
            str(_("Начало")): self.object.datebegin.strftime("%d.%m.%Y"),
            ...
            str(_("Валюта")): str(self.object.currency.code_char),
...
class ProjectUpdate(AddFilesMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context = super().get_context_data(**kwargs)
        context["header"] = _("Изменить Проект")
        # kwargs = super(ProjectUpdate, self).get_form_kwargs()
        kwargs = super().get_form_kwargs()
        context["files"] = ProjectFile.objects.filter(
            project_id=self.kwargs["pk"], is_active=True
        ).order_by("uname")
        # print(context)
        # print(kwargs)
        return context
        ...
        for mem in memb:
            ...
            if is_members_changed is False:
                if old_memb_list.count(mem) == 0:
                    is_members_changed = True
        historyjson = {
            str(_("Проект")): "" if self.object.name == old.name else self.object.name,
            str(_("Статус")): ""
            if self.object.status.name == old.status.name
            else self.object.status.name,
            str(_("Начало")): ""
            if self.object.datebegin == old.datebegin
            else self.object.datebegin.strftime("%d.%m.%Y"),
            str(_("Окончание")): ""
            if self.object.dateend == old.dateend
            else self.object.dateend.strftime("%d.%m.%Y"),
            ...
            else str(self.object.currency.code_char),
            str(_("Выполнен на, %")): ""
...
def tasks(request, projectid=0, pk=0):
    ...
    currentproject = (
        Project.objects.filter(id=projectid)
        .select_related(
            "company",
            "author",
            "assigner",
    ...
    if pk == 0:
        ...
        tree_task_id = 0
        if (
            currentuser == currentproject.author_id
            or currentuser == currentproject.assigner_id
        ):
            obj_files_rights = 1
    else:
        current_task = (
    ...
    if (
        ...
        button_project_create = _("Добавить")
        button_project_history = _("История")
        button_task_create = _("Добавить")
        if (
            currentuser == currentproject.author_id
            or currentuser == currentproject.assigner_id
        ):
            button_project_update = _("Изменить")
    ...
    return render(
        ...
        "project_detail.html",
        {
            ...
            "files": ProjectFile.objects.filter(
                project=currentproject, is_active=True
            ).order_by("uname"),
            "component_name": "projects",
            "objtype": "prj",
...
def taskcomments(request, taskid):
    ...
    minutes, sec = divmod(sec, 60)
    seconds = sec
    taskcomment_list = TaskComment.objects.filter(
        Q(author=request.user.id)
        | Q(
            task__project__members__in=[
                currentuser,
            ]
        ),
        is_active=True,
    ...
    return render(
        ...
        "task_detail.html",
        {
            ...
            "obj_files_rights": obj_files_rights,
            "files": ProjectFile.objects.filter(
                task=currenttask, is_active=True
            ).order_by("uname"),
            "objtype": "prjtsk",
            "media_path": settings.MEDIA_URL,
            "button_task_create": button_task_create,
...
def projectfilter(request):
    ...
    if prjstatus == "0":
        ...
        project_list = Project.objects.filter(
            ...
            company=companyid,
            dateclose__isnull=True,
        )  # .filter(id__in=[project.id for project in Project.objects.all() if project.is_leaf_node()])
    else:
        if prjstatus == "-1":
            # если в выпадающем списке выбрано "Все"
            project_list = Project.objects.filter(
    ...
    nodes = project_list.select_related(
        "company", "author", "assigner", "status", "type", "structure_type", "currency"
    ).distinct()
    object_message = ""
    if len(nodes) == 0:
        object_message = _("Проекты не найдены!")
...
def taskfilter(request):
    ...
    return render(
        request,
        "objects_list.html",
        {"nodes": nodes, "object_list": "task_list", "object_message": object_message},
    )
...
def projects_tasks(request):
   ...
   """
    ...
    projects_list = (
        ...
        .select_related("type", "status", "assigner", "author")
    ...
    projects_tasks_list = (
        ...
        .select_related("project", "type", "status", "assigner", "author")
        .order_by("dateend")
        .distinct()
    )
