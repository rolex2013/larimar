...
class ChatConsumer(WebsocketConsumer):
    def connect(self):
        ...
        # если тип чата - "Общий"
        if chat.type_id == 3:
            chatmember = ChatMember.objects.filter(
                chat_id=self.group_name, member_id=self.chat_member
            ).first()
            if chatmember == None:
                # print(chatmember)
                ChatMember.objects.create(
                    chat_id=self.group_name,
                    member_id=self.chat_member,
                    author_id=self.chat_member,
                    dateonline=datetime.now(),
                    datecurrent=datetime.now(),
                    dateoffline=datetime.now(),
                )
            else:
                ChatMember.objects.filter(
                    chat_id=self.group_name, member_id=self.chat_member
                ).update(dateonline=datetime.now())
        else:
            ChatMember.objects.filter(
                chat_id=self.group_name, member_id=self.chat_member
            ).update(dateonline=datetime.now())
    ...
    def disconnect(self, close_code):
        ...
        ChatMember.objects.filter(
            chat_id=self.group_name, member_id=self.chat_member
        ).update(dateoffline=datetime.now())
    ...
    def receive(self, text_data):
        ...
        if ismemberslist:
            ...
            memb = ChatMember.objects.filter(
                chat_id=self.group_name, member_id=self.chat_member
            ).first()
            memb.datecurrent = datetime.now()
            memb.save()
            chatmembers = ChatMember.objects.filter(
                chat_id=self.group_name, is_active=True
            ).order_by("-dateonline")
            # print(chatmembers)
    ...
    def notification_message(self, event):
        ...
        print(
            _("В чат chatid="),
            chatid,
            _("отправлено сообщение") + ' "' + message + '" (',
            formatDate,
            ")",
        )
