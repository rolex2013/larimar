            ...
            if self.cleaned_data["status"].id != self.initial["status"]:
                if self.cleaned_data["status"].is_close:  # == "Выполнен":
                    if self.user.id != self.initial["author"]:
                        ...
                        send_mail(
                            _("1YES! Ваш Клиент закрыт."),
                            _("Уведомляем о закрытии Вашего Клиента!"),
                ...
                objecttypeid = Meta_ObjectType.objects.get(shortname="clnt").id
                send_mail(
                    _("1YES! Вы назначены менеджером Клиента."),
                    _("Уведомляем о назначении Вам Клиента!"),
                    settings.EMAIL_HOST_USER,
                    [user_profile.email],
                )
                Notification.objects.create(
                    ...
                    text=_('Уведомляем о назначении Вам Клиента "')
...
class ClientTaskForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        members_list = list(
            Client.objects.filter(id=self.client).values_list("members", flat=True)
        )
        # print(members_list)
    ...
    def clean(self):
        ...
        if self.action == "update":
            if self.cleaned_data["status"].id != self.initial["status"]:
                ...
                objecttypeid = Meta_ObjectType.objects.get(shortname="tsk").id
                send_mail(
                    _("1YES! Вы назначены исполнителем События."),
                    _("Уведомляем о назначении Вам События!"),
                    settings.EMAIL_HOST_USER,
                    [user_profile.email],
                )
                Notification.objects.create(
                    ...
                    text=_('Уведомляем о назначении Вам События "')
                    + self.cleaned_data["name"]
                    + '".',
                    recipient_id=self.cleaned_data["assigner"],
                    sendto=user_profile.email,
