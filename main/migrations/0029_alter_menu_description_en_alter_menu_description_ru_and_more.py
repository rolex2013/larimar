# Generated by Django 4.1.7 on 2023-04-01 16:12

from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('main', '0028_alter_menuitem_description_en_and_more'),
    ]

    operations = [
        migrations.AlterField(
            model_name='menu',
            name='description_en',
            field=models.TextField(blank=True, null=True, verbose_name='Описание_en'),
        ),
        migrations.AlterField(
            model_name='menu',
            name='description_ru',
            field=models.TextField(blank=True, null=True, verbose_name='Описание_ru'),
        ),
        migrations.AlterField(
            model_name='menu',
            name='name_en',
            field=models.CharField(max_length=128, verbose_name='Наименование_en'),
        ),
        migrations.AlterField(
            model_name='menu',
            name='name_ru',
            field=models.CharField(max_length=128, verbose_name='Наименование_ru'),
        ),
    ]
