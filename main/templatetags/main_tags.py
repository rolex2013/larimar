...
def left_menu(context, menuid=0, is_auth=False):
    ...
    try:
        ...
        if is_auth:  # is True:
            ...
            if menuid == 0:
                nodes = MenuItem.objects.filter(
                    Q(menu_id=2)
                    & Q(is_active=True)
                    & (
                        Q(component_id__in=compid)
                        | Q(component_id__in=[])
                        | Q(component_id=1)
                    )
                )  # Главное (левое) меню
                ...
                nodes = MenuItem.objects.filter(
                    Q(menu_id=menuid)
                    & Q(is_active=True)
                    & (Q(component_id__in=compid) | Q(component_id=None))
                )
        else:
...
def group_name(context, is_auth=False):
    ...
    if is_auth:
        try:
            uccg = UserCompanyComponentGroup.objects.filter(
                user_id=context.request.user, component_id="1"
            ).order_by("-group_id")[0]
            groupname = '"' + uccg.group.name + '"'
        except:
            groupname = _('"Не назначено"')
    return groupname
...
def notifications(context, is_auth=False):
    ...
    if is_auth:
        ...
        metaobjecttype_list = (
            Meta_ObjectType.objects.filter(is_active=True).order_by("sort").distinct()
        )

    return {
        "nodes": nodes.select_related("author", "recipient", "objecttype")
        .order_by("datecreate")
        .distinct(),
        "status_selectid": "2",
        "metaobjecttype_list": metaobjecttype_list,
        "currentuserid": context.request.user.id,
            ...
            set(
                list(
                    UserCompanyComponentGroup.objects.filter(
                        ...
                        company__in=context.request.session["_auth_user_companies_id"],
                    ).values_list("user_id", flat=True)
                )
            )
        )
        nodes = (
            User.objects.filter(is_active=True, id__in=members)
            .exclude(id=context.request.user.id)
            .order_by("username")
            .distinct()
        )
